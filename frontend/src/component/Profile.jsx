    ...
    try {
      ...
      if (confirmDelete) {
        const response = await axios.delete(
          `https://blog-cards.up.railway.app/delete/${blogId}` || `http://localhost:3000/delete/${blogId}`
        );
        ...
        userData.blogs.map((blog) => (
          ...
          >
            <img
              ...
              src={`https://blog-cards.up.railway.app/images/${blog.blogPic}`}
              // src={`http://localhost:3000/images/${blog.blogPic}`}
              alt="Blog"
            />
            <div className="px-6 py-4">
              <div className="xl:flex xl:flex-row flex-col justify-between">
                <div className="font-bold text-lg xl:text-2xl">{blog.title}</div>
                <Link
                  to={`/edit/${blog._id}`}
